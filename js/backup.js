...
//  $('.outer').append($('<div class="row twitch_row loading">Loading</div>)'));
    ...
    $.ajax({
      ...
      timeout: 3000,
      type: "GET",
      dataType: "json", //if use jsonp, the ajax requests will be sent as a single request, and 'done' functions get done before ajaxEnd; if use json, each ajax request ends, and then 'done' functions get performed
//      beforeSend: function(){
...
function addFiveRows(data) {
  ...
  $('.loading').remove();
  $('.new_rows').show().removeClass("new_rows").addClass("one_set");
...
function makeRow(name, logo, url, status, desc) {
  ...
  $('.new_rows').append($('\
  <div class="row twitch_row ' + status + '"> \
    <div class="col-xs-1"> \
